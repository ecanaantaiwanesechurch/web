...
/* Events carousel, forked from Super's snippet - see VENDOR.md.
 * Coupled to carousel.css: this writes .active, those rules style it. */
...
(() => {
  ...
  const initCarousel = (gallery) => {
    ...
    const parent = gallery.closest('.notion-callout.bg-brown-light');
    if (!parent) { return; }
  ...
  const initializeAllGalleries = () => {
    document.querySelectorAll('.notion-collection-gallery').forEach(initCarousel);
  };

  // Super runs on Next.js: a soft nav swaps the DOM without a reload.
