...
(() => {
  ...
  function updateFooterIcons() {
    ...
    // Check if tithe.ly link already exists anywhere in the footer
    ...
    const addDonationLink = () => {
      ...
      giving.className = 'notion-link';

      // Append to the end
      footerIconContainer.appendChild(giving);
    };

    // Set up MutationObserver to watch for hydration completion
    const observer = new MutationObserver((mutations) => {
      ...
      // Check if we have real <a> tags (hydration complete)
      const realLinks = footerIconContainer.querySelectorAll('a.notion-link[href*="instagram"], a.notion-link[href*="facebook"]');

      if (realLinks.length >= 2 && !hydrationComplete) {
        hydrationComplete = true;

        // Wait a bit more to ensure all mutations are done
    ...
    observer.observe(footerIconContainer, {
      childList: true,
      subtree: true
    });

    // Fallback: if no hydration happens in 3 seconds, just add it
    setTimeout(() => {
